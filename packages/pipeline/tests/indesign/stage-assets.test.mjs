// The asset-staging stage: source (.idml/.pdf) → a generator-ready bundle on
...
//      what the generator (planAssets) will reference — i.e. the bundle is
...
test('runAssetStage writes a generator-ready bundle from an .idml (assets aligned with the generator)', async () => {
	...
	const work = await fs.mkdtemp(path.join(os.tmpdir(), 'flavian-stage-'));
	...
	try {
		...
		// the stage staged — so `--asset-dir <bundle>/assets` resolves it.
